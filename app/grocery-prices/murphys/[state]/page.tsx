...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `Murphy USA Grocery Prices in ${stateName} Today (${priceStr})`,
    description: `Murphy USA Grocery Prices in ${stateName} today. No membership needed. Based on ${stateName}'s current average of ${priceStr}, Murphy's saves drivers ${club.savingsLow}–${club.savingsHigh}¢/gal.`,
    keywords: [`murphy usa Grocery Price ${stateName.toLowerCase()}`, `murphy's gas in ${stateName.toLowerCase()}`, `murphy gas station ${stateName.toLowerCase()}`],
    openGraph: {
      title: `Murphy USA Grocery Prices in ${stateName} Today`,
      url: `https://whatsthegrocerybill.com/grocery-prices/murphys/${state}`,
      siteName: "What's the Grocery Bill?",
    },
    twitter: { card: 'summary', site: '@wtgbofficial' },
    alternates: { canonical: `https://whatsthegrocerybill.com/grocery-prices/murphys/${state}` },
  }
}

export default async function MurphysStatePage({ params }: Props) {
  const { state } = await params
