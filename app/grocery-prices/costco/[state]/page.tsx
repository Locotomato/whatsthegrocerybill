...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `Costco Grocery Prices in ${stateName} Today (${priceStr})`,
    description: `Costco grocery prices in ${stateName} today. Based on ${stateName}\'s current average of ${priceStr}. Estimated price, annual savings, and membership break-even.`,
    keywords: [`costco grocery price ${stateName.toLowerCase()}`, `costco grocery in ${stateName.toLowerCase()}`],
    openGraph: {
      title: `Costco Grocery Prices in ${stateName} Today`,
      url: `https://whatsthegrocerybill.com/grocery-prices/costco/${state}`,
      siteName: "What's the Grocery Bill?",
    },
    twitter: { card: 'summary', site: '@wtgbofficial' },
    alternates: { canonical: `https://whatsthegrocerybill.com/grocery-prices/costco/${state}` },
  }
}

export default async function CostcoStatePage({ params }: Props) {
  const { state } = await params
