...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `Walmart Grocery Prices in ${stateName} Today (${priceStr})`,
    description: `Walmart gas station prices in ${stateName} today. Based on ${stateName}'s current average of ${priceStr}, Walmart saves you ${club.savingsLow}–${club.savingsHigh}¢/gal. Estimated price, annual savings, and local context.`,
    keywords: [`walmart Grocery Price ${stateName.toLowerCase()}`, `what's the price of gas at walmart in ${stateName.toLowerCase()}`, `walmart ${stateName.toLowerCase()} grocery price`],
    openGraph: {
      title: `Walmart Grocery Prices in ${stateName} Today`,
      url: `https://whatsthegrocerybill.com/grocery-prices/walmart/${state}`,
      siteName: "What's the Grocery Bill?",
    },
    twitter: { card: 'summary', site: '@wtgbofficial' },
    alternates: { canonical: `https://whatsthegrocerybill.com/grocery-prices/walmart/${state}` },
  }
}

export default async function WalmartStatePage({ params }: Props) {
  const { state } = await params
