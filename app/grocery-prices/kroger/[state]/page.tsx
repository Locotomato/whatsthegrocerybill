...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `Kroger Grocery Prices in ${stateName} Today (${priceStr})`,
    description: `Kroger grocery prices in ${stateName} today. Based on ${stateName}\'s current average of ${priceStr}. Estimated price, annual savings, and membership break-even.`,
    keywords: [`kroger grocery price ${stateName.toLowerCase()}`, `kroger grocery in ${stateName.toLowerCase()}`],
    openGraph: {
      title: `Kroger Grocery Prices in ${stateName} Today`,
      url: `https://whatsthegrocerybill.com/grocery-prices/kroger/${state}`,
      siteName: "What's the Grocery Bill?",
    },
    twitter: { card: 'summary', site: '@wtgbofficial' },
    alternates: { canonical: `https://whatsthegrocerybill.com/grocery-prices/kroger/${state}` },
  }
}

export default async function KrogerStatePage({ params }: Props) {
  const { state } = await params
