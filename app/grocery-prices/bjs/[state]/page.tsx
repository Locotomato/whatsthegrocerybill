...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `BJ's Wholesale Club Grocery Prices in ${stateName} Today (${priceStr})`,
    description: `BJ's Grocery Prices in ${stateName} today. Based on ${stateName}'s current average of ${priceStr}, members save ${club.savingsLow}–${club.savingsHigh}¢/gal. Estimated price, annual savings, and membership break-even.`,
    keywords: [`bj's Grocery Price ${stateName.toLowerCase()}`, `bj's wholesale gas in ${stateName.toLowerCase()}`, `bj's grocery price ${stateName.toLowerCase()}`],
    openGraph: {
      title: `BJ's Grocery Prices in ${stateName} Today`,
      url: `https://whatsthegrocerybill.com/grocery-prices/bjs/${state}`,
      siteName: "What's the Grocery Bill?",
    },
    twitter: { card: 'summary', site: '@wtgbofficial' },
    alternates: { canonical: `https://whatsthegrocerybill.com/grocery-prices/bjs/${state}` },
  }
}

export default async function BJsStatePage({ params }: Props) {
  const { state } = await params
