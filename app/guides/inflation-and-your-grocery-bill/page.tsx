    ...
    <main style={{ background: 'var(--bg)', minHeight: '100vh', color: 'var(--text)' }}>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '40px 20px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: 12, marginBottom: 32 }}>
          {items.map(item => (
            <div key={item.name} style={{ background: '#fff', border: '1px solid var(--border)', borderRadius: 10, padding: '14px 16px' }}>
              <div style={{ fontSize: 12, color: 'var(--subtle)', marginBottom: 6 }}>{item.name}</div>
              <div style={{ fontSize: 22, fontWeight: 900, color: item.color }}>{item.change}</div>
              <div style={{ fontSize: 11, color: 'var(--muted)' }}>{item.period}</div>
            </div>
          ))}
        </div>
        ...
        <div style={{ marginTop: 32, paddingTop: 24, borderTop: '1px solid var(--border)', display: 'flex', gap: 16, flexWrap: 'wrap' }}>
          ...
          <Link href="/grocery-prices" style={{ color: 'var(--red)', fontSize: 13, textDecoration: 'none' }}>→ Prices by State</Link>
        </div>
      </div>
    </main>
  )
